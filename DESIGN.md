...
navigation are next, in Section 12). Both share the same pub/sub node architecture, the same
...
in advance; the vehicle localizes against them and doesn't build the map), 3D/terrain, and
...
history for evaluation and visualization, but it participates in the graph the same way any other
node would; it just happens to also be allowed to see ground truth.

## 3. Vehicle model

The vehicle is a kinematic bicycle model, the standard simplification for low-speed maneuvers (parking) where
...
steering angle. All angles are in **radians** throughout the codebase, since `theta'` compounds every step.
...
*Probabilistic Robotics*, ch. 7). Landmark positions come straight from `Environment`, so the filter
only estimates the vehicle's own pose against them (localization, not SLAM; see Section 1).
...
- **Landmark range-bearing** (nonlinear `h(x, landmark)`, standard range-bearing Jacobian,
  opportunistic, only when an obstacle is within sensor range). This is what makes it a genuine
...
- **Reeds-Shepp curves** (`planning/reeds_shepp.py`): the closed-form generalization of Dubins
  paths that also allows reverse gear. It implements the 4 circle-straight-circle (CSC) families
  `dubins.py` already has (LSL, RSR, LSR, RSL), each tried in both a forward and a backward-gear
  direction (8 candidates), plus the CCC ("3-point-turn") families (LRL/RLR), which
  `HybridAStarPlanner` deliberately leaves off (see Alternatives considered below and KNOWN_BUGS.md
  entry 5).
...
- **Hybrid A\*** (`planning/hybrid_astar.py`): search over a discretized `(x, y, theta)` state
  ...
  `VEHICLE_RADIUS` lives in `environment.py` so the planner and `harness._collided()` share one collision
  threshold (Section 8).
...
hazard, stop." \1. This was measured
...
between two close parked cars) collided with Pure Pursuit on 5/5 seeds. Two independent defects were
behind it. First, `brake_distance_for`'s `buffer` defaulted to exactly `HybridAStarPlanner.safety_margin`,
so `brake_distance` collapsed to precisely `VEHICLE_RADIUS`, the literal collision boundary, and the
emergency brake fired at the moment of contact rather than before it. Second, Pure Pursuit's
already-documented "no margin when curvature is already at the limit" weakness (Section 7). Widening
`safety_margin` only traded the collision for Pure Pursuit never converging, so it is not a tuning problem.

`ControllerNode` now runs a continuous, physics-based speed governor (`v_allowed = sqrt(2*a_max*gap)`,
recomputed every tick from the live sensor range) instead of a fixed trigger distance. Pure Pursuit now
records 0/5 collisions and 0/5 success (it stops short: curvature saturation with zero correction margin
is a tracking limitation the governor can't fix), MPC succeeds 5/5 (up to ~880 of a raised 1000-step
budget, since Hybrid A*'s avoidance routes are longer than M1's direct paths), and the other 9
(scenario, controller) pairs are unchanged at 5/5. The Pure Pursuit exception is documented rather than
hidden behind a numeric hack (`tests/test_simulation.py`'s `NEVER_SUCCEEDS`, pinned by its own regression
test). See `core/nodes/controller_node.py`'s module docstring and KNOWN_BUGS.md entry 2.
...
- **Keep the fixed Dubins path, just add obstacle checks**, rejected for the same reason the
  ...
  it."
- **Full 12-formula Reeds-Shepp (CCC included)**, rejected for M2 on a premise that proved wrong. CCC was
  added later but stays off for `HybridAStarPlanner` (`include_ccc=False`); see KNOWN_BUGS.md entry 5.
...
- Sensor dropout and latency are modeled only in parking mode, and latency beyond ~10 ticks still causes
  collisions (KNOWN_BUGS.md entry 7). Highway mode assumes every sensor reports every tick.
...
- The ego's collision footprint is a 1.0 m radius circle, smaller than the 4.5 x 1.8 m body drawn in the
...
- ~~Learned parking policy (RL)~~: built. `core/rl/parking_env.py`'s `ParkingEnv` (a Gymnasium
  ...
  and does it in ~65 steps versus the baseline's 261-365. It genuinely reaches the goal, reliably and
  safely, rather than just moving toward it. **Important caveat, not glossed over**: this isn't a fair fight
...
- **H3, Lane centering: done.** Stanley controller (`control/lane_centering.py`:
  ...
  error is measured at the *front axle*, not the vehicle's rear-axle reference point, so steering
corrects what's actually about to leave the lane. Brings the full 2D `Vehicle` bicycle model
  ...
  **Real lane geometry, not hand-authored**: `lane_centerline.csv` aggregates ~10,400 individual
  ...
  leader/follower excerpt), binned every 2m and lightly smoothed. The result has a genuine 1.56m end-to-end
...
- **Full closed-loop highway drive (H5), Phase A (H1+H2+H3 on one Vehicle): done.**
  ...
  **Real data, now the same NGSIM lane**: the lane centerline and the replayed leader are both NGSIM US-101
  **lane 2**. The earlier lane-1 leader and the low-speed-restart transient it exposed are in KNOWN_BUGS.md
  entry 6.
  ...
  **Three real bugs found building this, all in the H2 EKF, all invisible until now for the same
  ...
  really did carry over unchanged (checks: IMPLEMENTATION.md section 4).
...
- ~~Turning movements at a real 2D intersection~~: done (`control/intersection_geometry.py`,
  `intersection2d_harness.py`). A rare multi-vehicle wait cycle remains, safe but not always live; see
  KNOWN_BUGS.md entry 4.
