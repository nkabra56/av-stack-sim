...
tests/
  ...
  test_web_export.py        # viewer scene schema: track/signal lengths, KITTI frame flip
...
- **M6 (Tests)**: 280 tests across both modes run in ~340s. The 4 that need the optional `rl`/`viz`
  extras skip via `pytest.importorskip`, so 276 run on a base install.

## 4. Testing strategy

Current (280 tests, ~340s: up from ~100s pre-H5, almost entirely because `test_full_highway.py`
...
- `parallel_between_cars` (a reverse-gear cusp between two close cars) exposed Pure Pursuit's
  no-margin weakness at saturated curvature as a collision (5/5 seeds), unchanged by any
  `brake_distance` or `safety_margin` sweep. Part of the cause was a brake trigger that fired at the
  collision boundary; a physics-based speed governor fixed it, so Pure Pursuit now fails safe while
  MPC succeeds 5/5 (up to ~880 of a raised 1000-step cap). Full account: DESIGN.md section 6 and
...
- Residual drift *still* remained even with both fixes, isolated to: the highway EKF has no
  ...
  the real ~600m/78s NGSIM scenario. Fixed by having `HighwayVehicleNode` also publish a compass and a
  low-rate position fix, reusing the existing messages and EKF updates unchanged (DESIGN.md
  section 12).
